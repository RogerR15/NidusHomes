...
import { Trash2, MapPin, Edit, RefreshCcw, Loader2, Eye } from 'lucide-react'; // Am adaugat iconite noi
...
export default function MyListingsPage() {
    ...
    const [resettingId, setResettingId] = useState<number | null>(null); // State pentru loading la resetare
    ...
    const handleResetViews = async (id: number) => {
        ...
        try {
            ...
            await axios.put(`http://127.0.0.1:8000/listings/${id}/reset-views`, {}, {
                headers: {
                    Authorization: `Bearer ${session?.access_token}`
                }
            });
            ...
            <main className="max-w-5xl mx-auto py-10 px-4">
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {listings.map((item) => (
                            <div key={item.id} className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden hover:shadow-md transition flex flex-col">
